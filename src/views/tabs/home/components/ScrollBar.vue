...
onMounted(() => {
  ...
  const firstSwipeItem = container.chidlren[0]
  container.style.height = `${count * props.height}px`
  let index = 0
  useInterval(() => {
      index++
      // if exceed item numbers, move first element to the back
    if (index >= count) {
      firstSwipeItem.style.tranform = `translateY(${index * props.height}px)`
      // first element scrolling finish, reset location of container
